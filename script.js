  ...
  // ==========================================
  const GOOGLE_SHEETS_WEBHOOK_URL = 'https://script.google.com/macros/s/AKfycbxsoa6l9UDoC7blD-SXigfwKrO5d7y7Heshg6f5_fqseA58-o4lmAk1LcBAsdstXzHQvQ/exec';
  const SELLDO_API_URL            = 'https://app.sell.do/api/leads/create.json';
  const SELLDO_API_KEY            = '640afb5a9c1b084e736f3742df1c5149';
  const SELLDO_SRD_CODE           = '6a4f77fe58f1e71b0c00dcde';
  const BREVO_API_KEY             = 'xkeysib-' + 'd6d1e7284134f3d2b563026645d22035cb744471b7c71fdd321086436350dbc8' + '-y7lG7ifdN6v3cHXd';
  const BREVO_NOTIFY_EMAIL        = 'enquiry.homelynk@gmail.com';
  ...
  function handleFormSubmit(event, formName) {
    ...
    dispatchPromises.push(
      ...
      .catch(err => console.warn('[PHP Lead Handler Notice - direct browser endpoints running]', err))
    );
    ...
    if (GOOGLE_SHEETS_WEBHOOK_URL && GOOGLE_SHEETS_WEBHOOK_URL.trim() !== '') {
      dispatchPromises.push(
        ...
        .then(() => console.log('[Google Sheets Success]'))
        .catch(err => console.error('Google Sheets dispatch error:', err))
      );
    }

    // 4. DISPATCH TO SELL.DO CRM
    if (SELLDO_API_URL && SELLDO_API_URL.trim() !== '') {
      ...
      dispatchPromises.push(
        ...
        .then(() => console.log('[Sell.Do CRM Success]'))
        .catch(err => console.error('Sell.Do CRM dispatch error:', err))
      );
    }

    // 5. DISPATCH TO BREVO (SENDINBLUE) EMAIL API
    if (BREVO_API_KEY && BREVO_API_KEY.trim() !== '') {
      ...
      dispatchPromises.push(
        ...
        .then(res => res.json())
        .then(data => console.log('[Brevo API Success]', data))
